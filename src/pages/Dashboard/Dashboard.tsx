...
const Dashboard = () => {
    ...
    const mainContent = useMemo(() => {
        ...
        if (hasGroupedLists) {
            return displayMode === "tabs" ? renderTabsView() : renderSplitView();
        }
        return (
    ...
    return (
        <div className="flex flex-col bg-bg h-full overflow-hidden">
                ...
                {selectedList || showAiImport ? (
                    <>
                        <button
                            ...
                            onClick={showAiImport ? clearImport : clearSelectedList}
                            aria-label="Back"
                        >
                            <ChevronLeft size={20} />
                        </button>
                        <h1 className="flex-1 ml-3 text-[22px] font-extrabold text-text-strong tracking-tight">
                            {showAiImport ? "AI Shopping Assistant" : selectedList?.name}
                        </h1>
                    </>
                ) : (
                    <>
                        <div className="flex flex-col">
                            <h1 className="text-[22px] font-extrabold text-text-strong tracking-tight">My Lists</h1>
            ...
            <main className={`flex-1 p-7 max-w-[1200px] mx-auto w-full box-border max-[600px]:p-4 ${showAiImport ? "overflow-hidden flex flex-col" : "overflow-y-auto scrollbar-thin"}`}>
                {mainContent}
            </main>
