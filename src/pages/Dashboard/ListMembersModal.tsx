...
const ListMembersModal = ({
    ...
    const { shareList, removeCollaborator, leaveList, lists, changeCollaboratorRole } = useListsStore();
    ...
    const otherAdmins = collaborators.filter((c) => c.role === "ADMIN" && c.userId !== ownerId);
    const editors = collaborators.filter((c) => c.role === "EDITOR" && c.userId !== ownerId);
    const guests = collaborators.filter((c) => c.role === "GUEST" && c.userId !== ownerId);
    ...
    const handleRemove = async (collaborator: CollaboratorInfo) => {
        ...
        try {
            ...
            toast.success(`${collaborator.name || 'Member'} removed successfully`);
        } catch (err) {
    ...
    const handleRoleChange = async (userId: number, newRole: ListRole) => {
        ...
        const isSelf = targetCollaborator && currentUser && targetCollaborator.email === currentUser.email;

        if (isSelf && newRole !== "ADMIN") {
            if (otherAdmins.length <= 1) {
                toast.error("You cannot change your own role because you are the only admin collaborator on this list.");
                return;
            }
        }
    ...
    const renderCollaboratorRow = (collaborator: CollaboratorInfo) => {
        ...
        const canManage = isAdmin && collaborator.userId !== ownerId && (!isSelf || collaborator.role === "ADMIN");
                ...
                <div className="flex items-center gap-2 shrink-0">
                    {canManage ? (
                        <select
                            ...
                            onChange={(e) => handleRoleChange(collaborator.userId, e.target.value as ListRole)}
                            ...
                            <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-accent-subtle/40 border border-accent-border/20">
                                ...
                                <div className="flex flex-col min-w-0 flex-1">
                                    <span className="text-sm font-semibold text-text-strong truncate">
                                        {owner.name || "Unknown"} {currentUser && currentUser.email === owner.email && "(You)"}
                                    </span>
                                    <span className="text-xs text-text-muted truncate">
                                        {owner.email}
                                    </span>
